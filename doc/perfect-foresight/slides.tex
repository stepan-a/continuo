...
\title{Solving perfect-foresight models}
\subtitle{Crank--Nicolson collocation \emph{vs.}\ Dynare's stacked Newton}
...
\item \textbf{Continuous time} (\texttt{continuo}'s setting): the model
...
\item Equations split accordingly into $n_d$ \emph{dynamic rows}
  ...
  (pure equalities). \texttt{continuo} detects the partition
...
\item \textbf{Initial condition (states).} Each state $x_s$ ($s\in S$)
  ...
  \qquad s\in S.
  \]
  In \texttt{continuo} these values come from the \texttt{initval}
...
\item No algebraic variables here ($n_a=0$). In general \texttt{continuo}
...
\item \textbf{Why $(1,1)$ for \texttt{continuo}.} The smallest A-stable
...
\item The two coincide on linear problems and to leading order on
  smooth nonlinear ones. \texttt{continuo} uses the implicit midpoint
...
\item \texttt{continuo}'s model is implicit: $F(\dot x,x,e,\theta,t)=0$.
...
Non-zeros $O(n^2 N)$; each Newton step (next slide) hands this sparse
$J$ to a sparse direct solver that exploits the structure, so the solve
stays linear in $N$.
...
\item In \texttt{continuo}: SciPy's \texttt{spsolve} (sparse LU, SuperLU by
  default), CasADi-supplied exact sparse Jacobian, $\|G\|_\infty$
  convergence criterion ($10^{-10}$), max 50 iterations.\newline
...
\item A \textbf{surprise} corresponds to a change in the information
  set at some reveal time $t_r$. \texttt{continuo}'s orchestrator
...
\begin{tabular}{p{3.4cm}p{4.4cm}p{4.4cm}}
  \toprule
   & \textbf{Dynare} & \textbf{\texttt{continuo} (Crank--Nicolson)} \\
  \midrule
  ...
  Default solver & sparse direct (UMFPACK) & SciPy \texttt{spsolve} (SuperLU) \\
...
\item \textbf{Why does Dynare have a tridiagonal Jacobian but
  \texttt{continuo} only a bidiagonal one?} Because Crank--Nicolson
...
\item \textbf{Step-size flexibility.} In Dynare $dt=1$ period is
  ...
  frequency. In \texttt{continuo} $dt$ is a numerical knob: choose
...
\item \textbf{Symbolic differentiation.} Both pipelines need exact
  Jacobians. \texttt{continuo} uses CasADi (operator overloading,
...
\item \textbf{Crank--Nicolson} is the right default for
  ...
  \texttt{continuo} uses internally.\newline
